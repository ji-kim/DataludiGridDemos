...
<script>
    $(document).ready(function() {
        ...
        dsMain.onRowCountChanged = function(ds, count) {
            var s = ds.rowCount().toLocaleString();
            $("#rowCount").css("color", "blue").text(s);
        };

        // buttons
        $('#btnGetValue').click(function() {
            var index = treeMain.focusedIndex();
            if (treeMain.isValidCell(index)) {
                var v = dsMain.getValue(index.getDataIndex(treeMain), index.dataField());
                $_showMemo('conMain', 'getValue', v);
            }
        });
        $('#btnGetObject').click(function() {
            var row = treeMain.focusedDataRow();
            if (row) {
                var obj = dsMain.getRowObject(row);
                $_showMemo('conMain', 'getRowObject', obj);
            }
        });
        $('#btnGetObjects').click(function() {
            var rows = treeMain.getSelectedDataIndices();
            if (rows && rows.length > 0) {
                var objs = dsMain.getRowObjects(rows);
                $_showMemo('conMain', 'getRowObjects', objs);
            }
        });
        $('#btnGetAllObjects').click(function() {
            var rows = dsMain.getAllRows();
            if (rows && rows.length > 0) {
                var objs = dsMain.getRowObjects(rows);
                $_showMemo('conMain', 'getAllObjects', objs);
            }
        });
        $('#btnGetJsonRows').click(function() {
            var row = treeMain.focusedDataRow();
            if (row) {
                var rows = dsMain.getJsonRows(row);
                $_showMemo('conMain2', 'getJsonRows', rows);
            }
        });
        $('#btnGetDistinctValues').click(function() {
            var col = treeMain.focusedColumn();
            if (col) {
                var fld = col.dataIndex();
                var vals = dsMain.getDistinctValues(fld);
                $_showMemo('conMain2', 'getDistinctValues', vals);
            }
        });
    });
</script>
...
<body>
    ...
    <div>
        ...
        <span>필드 하나의 값을 가져온다. getValue를 참조한다.</span>
        <br>
        <button id="btnGetObject">Get Object</button>
        <span>한 행의 값들을 Json 객체로 가져온다. getRowObject를 참조한다.</span>
        <br>
        <button id="btnGetObjects">Get Objects</button>
        <span>한 행 이상을 각 행이 Json 객체인 배열로 가져온다. getRowObjects를 참조한다.</span>
        <br>
        <button id="btnGetAllObjects">Get All Objects</button>
        <span>전체 행을 가져온다. getAllRows와 getRowObjects 호출로 전체 행의 값들을 Json 객체 배열로 가져온다.</span>
    </div>
    <textarea id="conMain" class="helpMemo" style="width: 730px; min-height: 200px;"></textarea>
    <p>
    <div>
        <button id="btnGetJsonRows">Get Jason Rows</button>
        <span>지정한 데이터행의 자식행들을 json의 배열로 가져온다. 각 행의 자식행들은 하위 배열 속성으로 추가된다.</span>
        <br>
        <button id="btnGetDistinctValues">Get Distinct Field Values</button>
        <span>지정된 필드의 중복을 제거한 유일값들을 가져온다.</span>
    </div>
    <textarea id="conMain2" class="helpMemo" style="width: 730px; min-height: 200px;"></textarea>
</body>
</html>